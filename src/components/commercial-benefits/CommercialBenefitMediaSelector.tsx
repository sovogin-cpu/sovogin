...
import { listMediaItems } from "@/lib/media/media-repository";
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      listMediaItems(supabase, {
        ...
        .then(async (items) => {
          ...
          for (const item of items.slice(0, 30)) {
            if (classifyMediaType(item.mime_type) === "image") {
              try {
                const signed = await createCommercialBenefitSignedUrl(
                  supabase,
                  item.storage_path,
                  3600
                );
        ...
        .catch((err) => {
          console.error("Error al cargar imágenes para beneficio comercial:", err);
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            <span>{selectedMediaId ? "Cambiar Imagen" : "Elegir de Biblioteca"}</span>
          </button>
          <Link
            href="/admin/media"
            target="_blank"
