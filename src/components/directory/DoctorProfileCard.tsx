...
import { UserCheck, MapPin, Stethoscope, Video, ArrowRight } from "lucide-react";
import { DoctorDirectoryProfilePublic } from "@/lib/directory/types";
import { formatDoctorSpecialty } from "@/lib/directory/directory-utils";
      ...
      <div>
        ...
        <div className="h-56 w-full bg-slate-100 relative overflow-hidden flex items-center justify-center">
          ...
          {doctor.telemedicine_available && (
            <div className="absolute top-3 right-3 bg-emerald-600 text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-md flex items-center gap-1">
        ...
        <div className="p-6 space-y-3">
          ...
          <h3 className="font-extrabold text-slate-900 text-xl group-hover:text-[#006666] transition-colors leading-snug">
            <Link href={`/comunidad/directorio-medico/${doctor.id}`}>
              {doctor.display_name}
            </Link>
          </h3>
