...
export default function RegistrationPage() {
  ...
  const getPrice = () => {
    if (!event || !event.tiered_pricing?.tiers) return 0;
    const tiers = event.tiered_pricing.tiers;
    if (!Array.isArray(tiers)) return 0;
    const tier = tiers[formData.category_index];
    if (!tier) return 0;
    return formData.modality === 'presencial' ? tier.presencial : tier.virtual;
  };
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (price === 0) {
        ...
        alert(`Redirigiendo a pasarela de pago Wompi para: $${new Intl.NumberFormat('es-CO').format(price)} COP (${tier?.name || 'Inscripción'} - ${formData.modality})`);
        setIsSubmitting(false);
      }
    } catch (error: any) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="bg-white rounded-[3rem] overflow-hidden border border-slate-100 shadow-sm p-8 space-y-8">
              ...
              <div className="pt-8 border-t space-y-4">
                 ...
                 <div className="flex justify-between items-center text-slate-600">
                    <span className="capitalize">{formData.modality} - {event.tiered_pricing?.tiers?.[formData.category_index]?.name || "Seleccione"}</span>
                    <span className="font-bold text-slate-900">${new Intl.NumberFormat('es-CO').format(currentPrice)} COP</span>
                 </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8">
            <div className="bg-white rounded-[4rem] p-10 md:p-16 border border-slate-100 shadow-xl space-y-12">
              <div className="space-y-4">
                <h2 className="text-4xl font-bold text-slate-900 font-heading">Formulario de Inscripción</h2>
                <p className="text-slate-500">Complete sus datos para asegurar su cupo en el evento.</p>
              </div>

              <form onSubmit={handleRegister} className="space-y-10">
                {/* 1. Modality & Category Selection */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       {event.tiered_pricing?.tiers?.map((cat: any, idx: number) => (
                         <button
                           key={idx}
                           type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input required value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} placeholder="Ej: Dr. Juan Perez" className="h-14 pl-12 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white" />
                      </div>
                    </div>
                    <div className="space-y-2">
                ...
                <div className="pt-10 border-t flex flex-col md:flex-row items-center justify-between gap-8">
                   <div className="space-y-1">
                      <div className="text-slate-400 text-sm font-bold uppercase tracking-widest">Total a Pagar</div>
                      <div className="text-4xl font-bold text-slate-900 font-heading">
                        ${new Intl.NumberFormat('es-CO').format(currentPrice)} <span className="text-sm font-light text-slate-500">COP</span>
                      </div>
                   </div>
                   ...
                   >
                     {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin" /> : (
                       <>
                         {currentPrice === 0 ? "Completar Inscripción" : "Proceder al Pago"}
                ...
                <div className="flex items-center gap-2 text-slate-400 text-xs justify-center">
                  ...
                  Pago seguro procesado por Openpay. Sus datos están protegidos.
                </div>
              </form>
            </div>
          </div>
