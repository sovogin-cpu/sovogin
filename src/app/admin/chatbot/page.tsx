...
import { Bot, MessageSquare, Plus, Search, Trash2, Edit2, Loader2, BookOpen, Send } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-none shadow-sm rounded-3xl bg-primary text-white overflow-hidden relative group">
          <CardContent className="p-8">
            ...
            <p className="opacity-80">Asistente AI SOVOGIN</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-sm rounded-3xl bg-white overflow-hidden relative group border border-slate-50">
          <CardContent className="p-8">
            ...
            <p className="text-slate-500">Documentos de conocimiento</p>
          </CardContent>
        </Card>
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 font-heading">Base de Conocimientos</h2>
          <p className="text-slate-500 text-sm">Entrena a la IA con información específica de la asociación.</p>
        </div>
        
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <Button onClick={() => { setEditingId(null); setFormData({title:"", content:"", category:"General"}); setIsModalOpen(true); }} className="bg-primary hover:bg-primary/90 h-12 px-6 rounded-xl shadow-lg shadow-primary/20 gap-2 font-bold">
          ...
          <DialogContent className="sm:max-w-[600px] rounded-[2.5rem] border-none shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 pt-4">
              <div className="space-y-2">
                ...
                <Input required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="Ej: Requisitos de inscripción" className="rounded-xl h-12" />
              </div>
              <div className="space-y-2">
            ...
            <Card key={item.id} className="border-none shadow-sm rounded-2xl bg-white border border-slate-50 overflow-hidden">
              <CardContent className="p-6 flex items-start gap-4">
                ...
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="icon" onClick={() => openEditModal(item)} className="text-slate-400 hover:text-primary h-10 w-10 rounded-xl"><Edit2 className="w-4 h-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={() => deleteKnowledge(item.id)} className="text-slate-400 hover:text-red-500 h-10 w-10 rounded-xl"><Trash2 className="w-4 h-4" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
